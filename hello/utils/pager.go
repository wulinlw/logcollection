...
func fun4(po *PageOptions, totalpages int) string {
	...
	for i := totalpages - po.LinkItemCount; i <= totalpages; i++ {
		if po.Currentpage != i {
			...
			rs += "<li class=\"active\"><a href=\"" + po.Href + "&" + po.ParamName + "=" + con.Itoa(i) + "\">" + con.Itoa(i) + "<span class=\"sr-only\">(current)</span></a></li>"
		}
	}
...
func getHeader(po *PageOptions, totalpages int) string {
	var rs string = "<nav><ul class=\"pagination\">"
	if po.EnableFirstLastLink { //当首页,尾页都设定的时候,就显示

		rs += "<li><a href='" + po.Href + "&" + po.ParamName + "=" + con.Itoa(1) + "' aria-label=\"Previous\"><span aria-hidden=\"true\">" + po.FirstPageText + "</span></a></li>"
	...
	if po.EnablePreNexLink {
		...
		rs += "<li><a href='" + po.Href + "&" + po.ParamName + "=" + con.Itoa(a) + "' aria-label=\"Previous\"><span aria-hidden=\"true\">" + po.PrePageText + "</span></a></li>"
	}
	return rs
}
...
func getFooter(po *PageOptions, totalpages int) string {
	...
	if po.EnablePreNexLink {
		...
		rs += "<li><a href='" + po.Href + "&" + po.ParamName + "=" + con.Itoa(a) + "' aria-label=\"Next\"><span aria-hidden=\"true\">" + po.NextPageText + "</span></a></li>"
	}
	if po.EnableFirstLastLink { //当首页,尾页都设定的时候,就显示
		rs += "<li><a href='" + po.Href + "&" + po.ParamName + "=" + con.Itoa(totalpages) + "' aria-label=\"Next\"><span aria-hidden=\"true\">" + po.LastPageText + "</span></a></li>"
	}
	rs += "</ul></nav>"
	return rs
}
